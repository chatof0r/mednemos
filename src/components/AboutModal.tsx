...
export default function AboutModal({ onClose, onAdminClick }: AboutModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-brand dark:bg-charcoal border border-white/15 dark:border-brand/75 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto transition-colors">
        <div className="p-6 sm:p-8">
          ...
          {/* Theme toggle */}
          <div className="flex items-center justify-between p-3 rounded-xl border border-white/15 dark:border-brand/65 mb-5">
          ...
          {/* Disclaimer */}
          <div className="bg-white/5 border border-white/15 dark:border-brand/65 rounded-xl p-4 mb-6 text-sm space-y-2">
            ...
            <ul className="space-y-1 mt-2">
              {['Profil facultatif, données jamais revendues', 'Aucun bénéfice commercial', 'Contenu partagé entre étudiants'].map(t => (
          ...
          <div className="mb-5">
            ...
            <textarea
              ...
              rows={3}
              className="w-full border border-white/15 dark:border-brand/65 rounded-xl px-3 py-2 text-sm outline-none focus:border-white/40 dark:focus:border-brand/95 bg-white/5 text-ink placeholder:text-ink/20 resize-none transition-colors"
            />
            <button
              onClick={handleSend}
          ...
          {/* Admin */}
          <div className="border-t border-white/10 dark:border-brand/55 pt-4">
            <button
              onClick={() => { onClose(); onAdminClick(); }}
              className="w-full py-2 px-4 border border-white/15 dark:border-brand/65 rounded-xl text-sm text-ink/40 hover:text-ink/70 hover:border-white/30 dark:hover:border-brand/95 transition-colors flex items-center justify-center gap-2"
            >
